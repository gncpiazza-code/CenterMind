...
# ─── Utilidades de normalización de columnas ─────────────────────────────────
...
def _first_match(colnames: List[str], patterns: List[str]) -> str | None:
    normcols = [(c, _norm(c)) for c in colnames]
    pats = [_norm(p) for p in patterns]
    # Pasada 1: coincidencia exacta, por prioridad de patrón
    for p in pats:
        for c, nc in normcols:
            if nc == p:
                return c
    # Pasada 2: coincidencia parcial (contains), por prioridad de patrón
    for p in pats:
        for c, nc in normcols:
            if p in nc:
                return c
