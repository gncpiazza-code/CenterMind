-- DROP versiones viejas para evitar conflictos
...
-- CREAR VERSIÓN CON FALLBACK ERP (v7)
...
BEGIN
    IF p_date IS NOT NULL THEN
        ...
        SELECT 
            ...
            COALESCE(e.latitud_gps, erp.lat, erp.latitud)::numeric as lat,
            COALESCE(e.longitud_gps, erp.lon, erp.longitud)::numeric as lon,
            e.timestamp_subida,
            COALESCE(c.numero_cliente_local, erp.id_cliente_erp_local, '0')::text,
            COALESCE(c.nombre_fantasia, erp.nombre_fantasia, erp.nombre_cliente, 'Cliente S/N')::text,
        ...
        LEFT JOIN erp_clientes_raw erp ON (
            (c.numero_cliente_local = erp.id_cliente_erp_local AND e.id_distribuidor = erp.id_distribuidor)
            OR (e.id_cliente IS NULL AND erp.id_cliente_erp_local = 'REPLACE_ME' AND e.id_distribuidor = erp.id_distribuidor) -- Placeholder adjustment
        )
