...
export function Sidebar() {
  ...
  return (
    <aside
      className={`hidden md:flex flex-col h-screen overflow-hidden border-r px-3 py-8 gap-2 shrink-0 shadow-sm sticky top-0 transition-all duration-300 ease-in-out ${isCollapsed ? "w-20" : "w-64"}`}
      style={{
    ...
    >
      ...
      <div className="mt-auto space-y-4 shrink-0 pb-2">
        <div className="pt-4 border-t" style={{ borderColor: "var(--shelfy-border)" }}>
          {!isCollapsed && (
            <p
          ...
          {user && (
            <div className="flex flex-col gap-2 mb-2">
              <div
                className={`flex items-center gap-3 px-3 py-3 rounded-2xl border transition-all ${isCollapsed ? "justify-center" : ""}`}
                    ...
                    >
                      ...
                      {dists.map(d => (
                        <DropdownMenuItem
                          ...
                          className={`text-xs font-bold cursor-pointer ${user.id_distribuidor === d.id_distribuidor ? "text-violet-600 bg-violet-50" : ""}`}
                        >
          ...
          <button
            onClick={logout}
            className={`flex items-center gap-3 px-4 py-3 w-full text-sm font-bold rounded-2xl transition-all duration-200 active:scale-95 hover:bg-red-50 hover:text-red-500 ${isCollapsed ? "justify-center" : ""}`}
            style={{ color: "var(--shelfy-muted)" }}
          >
