...
interface CcSyncStatusBadgeProps {
  entry?: SyncStatusEntry;
  className?: string;
}

export function CcSyncStatusBadge({ entry, className }: CcSyncStatusBadgeProps) {
  ...
  const nextLabel = formatCcNextUpdateCountdown(entry?.next_run_at, now);
  ...
  return (
    ...
    >
      ...
      {nextLabel ? (
        <p className="inline-flex items-center gap-1 text-violet-700 dark:text-violet-300 font-semibold">
